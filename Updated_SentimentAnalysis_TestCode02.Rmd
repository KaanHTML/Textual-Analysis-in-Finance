...
text = fy2005$file.name.
new_column = sub(".*?2005(.*?)(.txt.*|$)", "\\1", text)
fy2005$accession_id <- new_column  
...
```{r}
#mofifying table further to get companies that are persistent in the sample from 2005-2020
#only get the accession_number that are also available in later years
#with this I ensure that each year we have the same companies and thus data is less prone to errors! 10 because 10 is the maxmimum count in one group
library(dplyr)
#I include only companies that are at least 9 years out of 12 in my sample (75%)
data_cleaned <- data %>%
  group_by(accession_id) %>%
  mutate(n = n())# %>%
  #filter(n >=10, Flesch_Index > 0)

#number of unique firms beofr merging with stock market data
data_cleaned %>%
  distinct(accession_id) %>%
  nrow()


#cleaning the CIK in data
data_cleaned$accession_id <- as.character(substring(data_cleaned$accession_id, 2, nchar(data_cleaned$accession_id)))

#cleaning accession number
...
#creating the monthly stock price development in a given year

combined_table <- as.data.frame(combined_table)

combined_table$StockPrice <- as.numeric(ifelse(combined_table$year == 2005, combined_table[,36], 
                                  ifelse(combined_table$year == 2006, combined_table[,37], 
...
```{r}
#adding the Russel Data data
combined_table_cleaned$Russell3000 <- ifelse(combined_table_cleaned$year == 2005, 8.76, ifelse(
  combined_table_cleaned$year == 2006, -6.4, ifelse(
    combined_table_cleaned$year == 2007, -46.4, ifelse(
      combined_table_cleaned$year == 2008, 59.6, ifelse(
        combined_table_cleaned$year == 2009, 18.6, ifelse(
          combined_table_cleaned$year == 2010, 4.7, ifelse(
            combined_table_cleaned$year == 2011, 10.8, ifelse(
              combined_table_cleaned$year == 2012, 25.2, ifelse(
                combined_table_cleaned$year == 2013, 10.7, ifelse(
                  combined_table_cleaned$year == 2014, -7.3, ifelse(
                    combined_table_cleaned$year == 2015, 22.6,ifelse(
                      combined_table_cleaned$year == 2016, 11.2,ifelse(
                        combined_table_cleaned$year == 2017, 5,ifelse(
                          combined_table_cleaned$year == 2019, 10.1,NA))))))))))))))

#creating binary dependent variable
combined_table_cleaned$MarketOutperformance <- ifelse(combined_table_cleaned$Russell3000 > combined_table_cleaned$StockPrice, 0, 1)
#combined_table_cleaned$MarketOutperformance <- as.factor(combined_table_cleaned$MarketOutperformance)
...
```{r}
#analysis of dictionary-based methods
...
```{}
# price development 
#modifying col_names: 2007 means for example the period from 2006-2007, and thus is referred to the FY2005 annual reports since they get available no earlier than 1/3/2006. Holding period 1 year (12months) from 1/3/2006-1/3/2007 for the example of FY2005 annaul reports.
for (i in 36:50) {
  colnames(combined_table)[i] = paste ("Year", substring(right(colnames(combined_table)[i], 5), 1,4), sep="")
}


combined_table$Stock_Price_Development <- ifelse(combined_table$year == 2005, combined_table$Year2007, ifelse(
  combined_table$year == 2006, combined_table$Year2008, ifelse(
    combined_table$year == 2007, combined_table$Year2009, ifelse(
      combined_table$year == 2008, combined_table$Year2010, ifelse(
        combined_table$year == 2009, combined_table$Year2011, ifelse(
          combined_table$year == 2010, combined_table$Year2012, ifelse(
            combined_table$year == 2011, combined_table$Year2013, ifelse(
              combined_table$year == 2012, combined_table$Year2014, ifelse(
                combined_table$year == 2013, combined_table$Year2015, ifelse(
                  combined_table$year == 2014, combined_table$Year2016, NA))))))))))
...
data %>%
  filter(Fog_Index < 40) %>%
  group_by(year) %>%
  summarise(FOG_mean = mean(Fog_Index), n= n()) %>%
  ggplot(aes(year, FOG_mean))+
  geom_line()
...
data_1 %>%
  ...
  ggplot(aes(year, FOG_mean))+
  geom_line()
#ggplot(test_02, aes(x=year, y=FOG_mean)) +
 #   geom_line()
...
```{}
#some statistics
table_1 <- data_cleaned %>%
	group_by(year) %>%
	summarise(mean_run = mean((file.size.)/1000000))
  
write.csv(table_1, file = "table_1.csv")

#par(family="Times")
windowsFonts(A = windowsFont("Times New Roman"))
ggplot(data = table_1, aes(x=as.integer(year), y= mean_run))+
    geom_line() +
    theme_classic() +
    theme(text = element_text(family = "A", size = 16))+
    labs(y="Average File Size (mio kilobyte)", x = "Year")+
    ggsave("Plot_FileSizeOverYears.jpeg", plot = last_plot(), device = NULL, path = NULL,
       scale = 1, width = 10, height = 6, dpi = 300, limitsize = TRUE, units = c("in", "cm", "mm"))

#compare average word length in 2012 in a graph --> relationship also holds for the other years!
plot1 <- ggplot(data=data_cleaned[data_cleaned$year==2012,], aes(x=log(file.size.), y=log(number.of.words.))) +
  geom_jitter(alpha=1) +
  geom_smooth(method="lm") +
  theme_classic()+
  theme(text = element_text(family = "A", size = 16))+
  labs(y="Average number of words (log. value)", x = "Average file size (log. value)")+
  ggsave("FileSize_NumberWords.jpeg", plot = last_plot(), device = NULL, path = NULL,
       scale = 1, width = 10, height = 6, dpi = 300, limitsize = TRUE, units = c("in", "cm", "mm"))



#only continuing with the companies that persist in the following years
#merged_table <- merge(x = fy2005, y = fy2006, by = "accession_id")
#merged_table <- merge(x = merged_tabe, y = fy2007, by = "accession_id")
```


```{}
#manually write S&P 500 perfromance in thtat year



#testing if stock outperformed market: 0 if did not beat SP500, and 1 if id did beat the index
combined_table$MarketOutperformance <- ifelse(combined_table$SP500 > combined_table$Stock_Price_Development, 0, 1)
combined_table$MarketOutperformance <- as.factor(combined_table$MarketOutperformance)
#
combined_table$X..uncertainty.lagged <- lag(combined_table$X..uncertainty., 1)

combined_table$X..negative..lagged <- lag(combined_table$X..negative., 1)

combined_table$X..positive..lagged <- lag(combined_table$X..positive., 1)

combined_table$change_uncertainty <- combined_table$X..uncertainty. - combined_table$X..uncertainty.lagged

combined_table$change_negative <- combined_table$X..negative. - combined_table$X..negative..lagged

combined_table$change_positive <- combined_table$X..positive. - combined_table$X..positive..lagged

combined_table[is.na(combined_table)] = 0


CATCompany_Example <- filter(combined_table, CIK_Clean == "0000018230")

'combined_table %>%
  filter(as.numeric(Stock_Price_Development) < 100) %>%
  ggplot(aes(x=log(file.size.), y=(as.numeric(Stock_Price_Development)), color = MarketOutperformance)) + 
  geom_point(method = "lm")
'

combined_table$Percentile_Negative <-  percent_rank(combined_table$X..negative.)
#combined_table$Percentile_Negative

combined_table$Percentile_Negative <- ifelse(combined_table$Percentile_Negative < 0.1, 1,
                                             ifelse(combined_table$Percentile_Negative >= 0.10 & combined_table$Percentile_Negative < 0.20, 2, 
                                                    ifelse(combined_table$Percentile_Negative >= 0.20 & 
                                                                  combined_table$Percentile_Negative < 0.30, 3, 
                                                                    ifelse(combined_table$Percentile_Negative >=0.30  & combined_table$Percentile_Negative <0.4, 4, ifelse(combined_table$Percentile_Negative >=0.4 & combined_table$Percentile_Negative < 0.5, 5, ifelse(combined_table$Percentile_Negative >=0.5 & combined_table$Percentile_Negative < 0.6, 6, ifelse(combined_table$Percentile_Negative >=0.6 & combined_table$Percentile_Negative < 0.7, 7, ifelse(combined_table$Percentile_Negative >=0.7 & combined_table$Percentile_Negative < 0.8, 8, ifelse(combined_table$Percentile_Negative >= 0.8 & combined_table$Percentile_Negative < 0.9, 9, ifelse(combined_table$Percentile_Negative >=0.9, 10,NA))))))))))

combined_table$Percentile_Negative <- as.factor(combined_table$Percentile_Negative)
#combined_table$Percentile_Negative
...
#str(combined_table)
#combined_table_n <- as.data.frame(lapply(combined_table[1:ncol(combined_table)], normalize))
```
...
combined_table %>%
  group_by(year) %>%
  summarise(average_modal_weak = mean(X..modal.weak.)) %>%
  ggplot(aes(year,average_modal_weak)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(average_litigious_words = mean(X..litigious.)) %>%
  ggplot(aes(year,average_litigious_words)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(average_file.size. = mean(file.size.)) %>%
  ggplot(aes(year,average_file.size.)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(number.of.words. = mean(number.of.words.)) %>%
  ggplot(aes(year,number.of.words.)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(positive_words = mean(X..positive.)) %>%
  ggplot(aes(year,positive_words)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(negative_words = mean(X..negative.)) %>%
  ggplot(aes(year,negative_words)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(uncertainty_words = mean(X..uncertainty.)) %>%
  ggplot(aes(year,uncertainty_words)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(strong_modal_word = mean(X..modal.strong.)) %>%
  ggplot(aes(year,strong_modal_word)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(constraining = mean(X..constraining.)) %>%
  ggplot(aes(year,constraining)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(alphabetic = mean(X..of.alphabetic.)) %>%
  ggplot(aes(year,alphabetic)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(complex_words = mean(avg...of.syllables.per.word.)) %>%
  ggplot(aes(year,complex_words)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(word_length = mean(average.word.length.)) %>%
  ggplot(aes(year,word_length)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(vocabulary_used = mean(vocabulary)) %>%
  ggplot(aes(year,vocabulary_used)) + 
    geom_line()
...
#change in negativity kann vorhersagen ob es kracht!!
combined_table %>%
  group_by(year) %>%
  summarise(negative_change = mean(change_negative)) %>%
  ggplot(aes(year,negative_change)) + 
    geom_line()

combined_table %>%
  group_by(year) %>%
  summarise(psoitive_change = mean(change_positive)) %>%
  ggplot(aes(year,psoitive_change)) + 
    geom_line()

summary(combined_table$Percentile_Negative)


combined_table$Stock_Price_Development <- ifelse(combined_table$Stock_Price_Development == "-", 0,combined_table$Stock_Price_Development)

#positiver wert bei change_negative heißt, dass die Anzahl der negativen Wörter gestiegen ist 
combined_table %>%
  group_by(MarketOutperformance, year) %>%
  summarise(mean_change_negative_words = mean(change_negative, na.rm = T))%>%
  ggplot(aes(x=year, y=mean_change_negative_words, color=MarketOutperformance)) +
    geom_line()+
  ggsave("ChangeNegativeWords.jpeg", plot = last_plot(), device = NULL, path = NULL,
       scale = 1, width = 10, height = 6, dpi = 300, limitsize = TRUE, units = c("in", "cm", "mm"))
...
#creating similar to LM Quantiles of negative words
combined_table$Negative_Quantiles <- ifelse(combined_table$X..negative. >= quantile(combined_table$X..negative.)[1] &
                                                     combined_table$X..negative. < quantile(combined_table$X..negative.)[2], 1,
                                                   ifelse(combined_table$X..negative. >= quantile(combined_table$X..negative.)[2] & 
                                                            combined_table$X..negative. < 
                                                            quantile(combined_table$X..negative.)[3], 2,
                                                          ifelse(combined_table$X..negative. >= quantile(combined_table$X..negative.)[3] &
                                                                   combined_table$X..negative. < quantile(combined_table$X..negative.)[4], 3, 
                                                                 ifelse(combined_table$X..negative. >= quantile(combined_table$X..negative.)[4] &
                                                                   combined_table$X..negative. < quantile(combined_table$X..negative.)[5], 4,
                                                                   ifelse(combined_table$X..negative. >=
                                                                            quantile(combined_table$X..negative.)[5], 5, NA)))))



##kannst du so nicth rechnern, da du in jedem Jahr die Quintiles ausrechnenn musst und danach sortieren!

combined_table$Negative_Quantiles <- as.factor(combined_table$Negative_Quantiles)                                                            

ggplot(combined_table, aes(x=Negative_Quantiles, y=Stock_Price_Development)) +
  geom_point()

combined_table %>%
  group_by(year, Negative_Quantiles) %>%
  summarise(mean = mean(Stock_Price_Development)) %>%
  ggplot(aes(x=year, y=mean)) + 
  geom_point()

combined_table %>%
  filter(Negative_Quantiles==1) %>%
  group_by(year) %>%
  summarise(mean = mean(Stock_Price_Development, na.rm = T)) %>%
  ggplot(aes(x=year,y=mean)) + 
    geom_line()



qqplot(combined_table$Stock_Price_Development,combined_table$X..negative., main="Plot")#

combined_table %>%
  group_by(year) %>%
  summarise(n = n())

lm_model <- lm(MarketOutperformance ~., data = combined_table)
